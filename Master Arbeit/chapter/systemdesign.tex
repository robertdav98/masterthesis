...
\end{itemize}
Nachdem der Code von der App gescannt wurde, wird die Anfrage angezeigt. Ebenso wird die URL des Issuers angezeigt und der Name des Credentials. Der Holder initiert die Verifizierung durch das Drücken des "Approve"-Buttons. Dadurch kontaktiert die App die URL, die im Request-Body unter 'callbackUrl' angegeben wurde. An dieser Stelle führt der Server die tatsächliche Verifikation aus. Als Ergebnis wird ein zk-Proof erstellt und abgespeichert. Der Ablauf hierfür wird im folgenden Code beschrieben:
